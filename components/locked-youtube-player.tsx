...
export function LockedYouTubePlayer({ videoId, startLabel, onWatchProgress }: Props) {
  ...
  const skipBySeconds = useCallback(
    (delta: number) => {
      ...
      window.setTimeout(() => {
        ...
        try {
          ...
          if (Number.isFinite(actual)) setCurrent(actual);
        } catch {
          /* ignore */
        }
      }, 150);
    ...
    [current, duration, revealControls]
  );

  const handleSurfaceTap = useCallback(
    (clientX: number, width: number) => {
      const ratio = width > 0 ? clientX / width : 0.5;
      ...
      if (isDoubleTap && zone === "left") {
        skipBySeconds(-SEEK_STEP_SECONDS);
    ...
    [controlsVisible, revealControls, skipBySeconds, togglePlay]
  );
  ...
  }, [started, skipBySeconds, togglePlay]);
          ...
          <>
            <button
              ...
              onClick={(e) => {
                ...
                handleSurfaceTap(e.clientX - rect.left, rect.width);
              }}
              onTouchEnd={(e) => {
                e.preventDefault();
                ...
                if (!touch) return;
                const rect = e.currentTarget.getBoundingClientRect();
                handleSurfaceTap(touch.clientX - rect.left, rect.width);
                window.setTimeout(() => {
            ...
            {seekHint !== null && (
              <div
                className={`pointer-events-none absolute top-1/2 z-40 -translate-y-1/2 rounded-full bg-black/55 px-4 py-3 text-white shadow-lg ${
                  seekHint < 0 ? "left-[18%] -translate-x-1/2" : "right-[18%] translate-x-1/2"
                }`}
              >
                <span className="text-lg font-semibold tabular-nums">
                  {seekHint > 0 ? `+${seekHint}` : seekHint} s
                </span>
              </div>
            )}
